...
class FlxTransitionableState extends FlxState
{
	 ...
	 */
	public function transitionIn(?isFake:Bool = false, ?doInEnd:Void->Void):Void
	{
		if (transIn != null && transIn.type != NONE)
		{
            ...
            _trans.cameras = [camTrans];

			_trans.setStatus(FULL);
			openSubState(_trans);

			_trans.finishCallback = isFake ? function (){transitionOut(doInEnd);} :finishTransIn;
			_trans.start(OUT);
		}
	}
