...
class TransitionFade extends TransitionEffect
{
	...
	public function new(data:TransitionData)
	{
		...
		add(back);
        arriba = new FlxSprite().makeGraphic(1,1,0);
        arriba.scrollFactor.set();
        arriba.setGraphicSize(4000,9000);
        arriba.updateHitbox();
        add(arriba);
	}
    ...
    override function update(elapsed:Float)
    {
        ...
        arriba.y = back.y  + (_data.direction.y > 0 ? -back.height: back.height);
    }
