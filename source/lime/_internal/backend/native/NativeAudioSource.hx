...
class NativeAudioSource
{
	...
	public function setCurrentTime(value:Int):Int
	{
		...
		if (playing)
		{
			...
			var timeRemaining = Math.floor((getLength() - value) / get_pit());
	...
	public function setLength(value:Int):Int
	{
		if (playing && length != value)
		{
			...
			var timeRemaining = Math.floor((value - getCurrentTime()) / get_pit());
    ...
    /**
    * I created this, but psych engine is already with this, whhen im crying now
    */
	public function get_pit():Float
    {
        if (handle != null)
		...
		if (playing)
			{
				...
				var timeRemaining = Math.floor((getLength() - getCurrentTime()) / v);
    ...
    public function set_pit(v:Float):Float
	{
		set_speed(v);
		
		if (handle != null)
			AL.sourcef(handle, AL.PITCH, v);
